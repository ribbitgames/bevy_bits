            ...
            match command {
                BoardCommand::Swap(pos1, pos2) => {
                    if board.swap(pos1, pos2).is_ok() {
                        if let Err(err) = events.push(BoardEvent::Swapped(pos1, pos2)) {
                            error!("{err}");
                            continue;
                        }
                        ...
                        if let Err(err) = events.push(BoardEvent::Matched(matches)) {
                            error!("{err}");
                            continue;
                        }
                    } else if let Err(err) = events.push(BoardEvent::FailedSwap(pos1, pos2)) {
                        error!("{err}");
                        continue;
                    };
                }
                BoardCommand::Pop(gems) => {
                    for gem in &gems {
                        board.remove(*gem);
                        if let Err(err) = events.push(BoardEvent::Popped(*gem)) {
                            error!("{err}");
                            continue;
                        }
                    }
                    let mut drops: Vec<Drop> = board
                        .drop()
                        .iter()
                        .copied()
                    ...
                    if let Err(err) = events.push(BoardEvent::Dropped(drops)) {
                        error!("{err}");
                        continue;
                    }

                    if let Err(err) =
                        events.push(BoardEvent::Spawned(board.fill().iter().copied().collect()))
                    {
                        error!("{err}");
                        continue;
                    }
                ...
                BoardCommand::Shuffle => {
                    ...
                    if let Err(err) = events.push(BoardEvent::Shuffled(moves)) {
                        error!("{err}");
                        continue;
                    }

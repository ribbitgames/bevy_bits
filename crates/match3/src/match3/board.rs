...
use super::mat::{Match, MatchDirection, Matches};
            ...
            for y in (0..self.dimensions.y).rev() {
                if self.get([x, y].into()).is_none() {
                    ...
                    for above in (0..y).rev() {
                        if let Some(typ) = self.get([x, above].into()).copied() {
        ...
        for x in 0..self.dimensions.x {
            for y in 0..self.dimensions.y {
                ...
                if self.get(pos.into()).is_none() {
                    let Some(new_type) = self
                        .types
                        .iter()
                        .nth(fastrand::usize(..self.types.len()))
                        .copied()
                    else {
    ...
    fn straight_matches(&self, direction: MatchDirection) -> Matches {
        ...
        for one in match direction {
            ...
            for two in match direction {
                ...
                let current_type = self.get(pos);

                if current_match.is_empty() || previous_type == current_type {
                    previous_type = current_type;
                    current_match.push(pos);
                } else if previous_type != current_type {
                    match current_match.len() {
                        0..=2 => {}
                        _ => matches.add(Match::Straight(current_match.iter().copied().collect())),
                    }
...
#[expect(dead_code)] // NoGem is used in tests.
pub enum SwapError {
    NoGem(UVec2),
    NoMatches,
}
...
mod tests {
    ...
    use super::Board;
    use crate::{BoardMove, Matches, SwapError};
